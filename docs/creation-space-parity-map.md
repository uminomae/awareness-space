...
| reports 導線 | `src/reports/`, `index.html` reports section, `transform/reports/README.md` | `src/reports/`, `index.html` reports section | 進行中 | domains / status modal は追随中。新 UI を追加するときは reports との干渉確認が必要。2026-03-24 に `transform/reports/README.md` から D01-D30 記述を除去し、awareness 固有スコープへ是正済み。README から `src/reports/*` と `#reports-section` へ辿れる入口を追加し、進捗 legend / filter、有効な ID / status / progress metrics も awareness 側で有効化済み |
...
| domains workflow | `transform/domains/`, `transform/scripts/build-pdf-guide.sh`, `transform/scripts/publish-awareness-domains.sh` | `transform/domains/`, `transform/scripts/build-pdf-guide.sh` | 概ね整備済み | 追加実装時は `transform/domains/WORKFLOW.md` との差分検査を先に行う。2026-03-24 に `transform/domains/README.md` へ既存公開物の配置先を明記済み。`WORKFLOW.md` も Git 規約と publish 対象に合わせて更新済み |
