...
| topics workflow | `transform/topics/`, `transform/scripts/build-pdf-guide.sh`, `transform/scripts/publish-awareness-topics.sh`, `transform/scripts/generate-awareness-topic-svg.sh` | `transform/topics/`, `transform/scripts/build-pdf-guide.sh`, `transform/scripts/generate-svg.sh` | 進行中 | 追加実装時は `transform/topics/WORKFLOW.md` との差分検査を先に行う。2026-03-26 に `survival-trust-axis` 向け 1 topic 1 SVG の最小 workflow を導入済み。次は `four-layers` への横展開と publish 契約の拡張を進める |
