...
| reports 導線 | `src/reports/`, `index.html` reports section, `transform/reports/README.md` | `src/reports/`, `index.html` reports section | 進行中 | topics / status modal は追随中。新 UI を追加するときは reports との干渉確認が必要。2026-03-24 に `transform/reports/README.md` から D01-D30 記述を除去し、awareness 固有スコープへ是正済み。README から `src/reports/*` と `#reports-section` へ辿れる入口を追加し、進捗 legend / filter、有効な ID / status / progress metrics も awareness 側で有効化済み |
...
| topics workflow | `transform/topics/`, `transform/scripts/build-pdf-guide.sh`, `transform/scripts/publish-awareness-topics.sh`, `transform/scripts/generate-awareness-topic-svg.sh` | `transform/topics/`, `transform/scripts/build-pdf-guide.sh`, `transform/scripts/generate-svg.sh` | 概ね整備済み | 追加実装時は `transform/topics/WORKFLOW.md` との差分検査を先に行う。2026-03-26 に `survival-trust-axis` と `four-layers` へ 1 topic 1 SVG の最小 workflow を導入済み。次は publish 契約の拡張や複数 SVG 対応を必要に応じて進める |
...
| dev / runtime 補助 | `src/main.js`, `src/main-runtime-context.js`, `src/main-bootstrap.js`, `src/main-dev-runtime.js`, `src/dev-*`, `src/background-mode.js` | `src/main.js`, `src/main-runtime-context.js`, `src/main-bootstrap.js`, `src/main-dev-runtime.js`, `src/dev-*`, `src/background-mode.js` | 進行中 | shell chrome を追加するときは dev mode 表示と競合させない。2026-03-24 に runtime context、main bootstrap、dev chrome 補助を `main.js` から切り出し、creation-space に近い責務分離へ寄せ始めた |
