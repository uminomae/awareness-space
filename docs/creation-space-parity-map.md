...
| reports 導線 | `src/reports/`, `index.html` reports section, `transform/reports/README.md` | `src/reports/`, `index.html` reports section | 進行中 | domains / status modal は追随中。新 UI を追加するときは reports との干渉確認が必要。2026-03-24 に `transform/reports/README.md` から D01-D30 記述を除去し、awareness 固有スコープへ是正済み。README から `src/reports/*` と `#reports-section` へ辿れる入口を追加し、進捗 legend / filter も awareness 側で有効化済み |
