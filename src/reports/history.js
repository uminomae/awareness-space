...
function getDomainIdFromHash(hash = window.location.hash) {
...
export function getDomainIdFromUrl(search = window.location.search, hash = window.location.hash) {
    try {
        const params = new URLSearchParams(search);
        return normalizeDomainId(params.get(DOMAIN_QUERY_PARAM)) || getDomainIdFromHash(hash);
    } catch {
        return getDomainIdFromHash(hash);
    }
}

function hasDomainQueryParam(search = window.location.search, hash = window.location.hash) {
    try {
        return new URLSearchParams(search).has(DOMAIN_QUERY_PARAM) || Boolean(getDomainIdFromHash(hash));
    } catch {
        return Boolean(getDomainIdFromHash(hash));
    }
}
...
export function getDomainHistoryMarker(historyState = window.history?.state, domainId = '') {
    ...
    return { domainId: normalizedId, mode: normalizedMode };
}
...
export function createReportsHistoryController({
    ...
    function syncDomainModalWithUrl({
        ...
        if (state.activeDomainId === domainId && isMdModalVisible()) {
            ...
            clearPendingDomainSync();
            if (treatAsInitial) {
                updateDomainHistoryEntry(domainId, { method: 'replace', mode: DOMAIN_HISTORY_MODE_INITIAL });
            }
            return;
        }
    ...
    function handleDomainPopState(event) {
        ...
        if (!state.reportsReady) {
            ...
            if (!domainId) clearPendingDomainSync();
            return;
        }
