# survey 5W1H source map

`awareness-space` の survey-status.md を書くために、
「何を、なぜ、誰に、いつ、どこで、どう調査したのか」を
既存の正本と関連 Issue から回収したメモ。

## What

調査対象は、意識モデルをどの単位で記述できるかである。
現時点では以下を主要構成要素として扱う。
...
目的は、既存の理論資産を単純移設することではなく、
「意識とは何か」を記述するための新しい探索を立ち上げることにある。
生存-信頼軸を起点仮説とし、そこから意識モデルの構造を再編する。

### source

- `awareness-space/evidence/PROJECT.md`
- `kesson-driven-thinking#280` コメント「原則の修正」「開始時点の仮説」

## Who

設計判断主体は pjdhiro。
repo 内実装は Codex が担う。
survey の読者は、`awareness-space` を訪れて
「この repo では何をどのように調査しているのか」をまだ知らない人である。

### source

- `awareness-space/PROJECT.md`
- `awareness-space/docs/issue-management.md`
- `creation-space/transform/survey/reader-rules/reader-rules-creation-survey.md`

## When

`awareness-space` は 2026-03 に開始。
現時点では bootstrap 後の構造化フェーズであり、
guide / 調査内容 / report の三層を整備している。

### source

- `awareness-space/evidence/PROJECT.md`
- `kesson-driven-thinking#280` コメント「PM整理」「bootstrap 完了」
