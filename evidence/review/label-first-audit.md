...
### F1. `Withhold` が独立 report を持つことで、概念の未確定さより概念の自立性が先に見える
...
- report が存在するため、読者からは `Withhold` が既に独立概念として成立しているように見える
- report 本文でも境界や反例より先に「最小定義」が置かれている
- manifest でも `Withhold` が他の構成要素と同格の `report_ready` 項目として固定されている

暫定判定:

- `Withhold` は「独立 report を維持してよい概念」ではなく、
  「概念監査待ちの仮ラベル」として降格候補
...
- 「現在公開済みの論拠」に `Withhold report` を含めるため、読者は `Withhold` を主要な公開済み概念として読む
...
- `Withhold report` を他の report と横並びで reader-facing な論拠として提示している
- 「生存-信頼軸、4層モデル、M1、Concept Notes、Withhold にはすでに公開済み report がある」という記述が、
  調査成熟度の差を隠している
...
- guide は一度弱める必要がある
- 特に `Withhold` は独立論拠ではなく「要再調査の仮説ラベル」として表現を落とす必要がある
...
- rule が最初から `生存-信頼軸 / 4層モデル / Withhold / concept notes` を対象コンテンツとして固定している
...
- 少なくとも `Withhold` の独立 card / independent domain 扱いは凍結対象
...
- 「次の実装候補」でも `生存-信頼軸 / 4層モデル / Withhold のうち、どれを中核 report とするか` を問うている
...
- 独立論拠としての見せ方を弱める
- `Withhold` を中心構成要素ではなく仮ラベルとして表現する
- 調査中 / 概念監査待ちの表示を優先する
...
4. `Withhold` を含む既存ラベルを、調査途中ではどう凍結するか
5. survey / guide / manifest を、調査再起動期にどう見せるか

## 6. 暫定結論

最も大きい問題は、`Withhold` が「間違っている」ことそのものではない。
問題は、`Withhold` が **まだ境界未確定の探索ラベルであるにもかかわらず、
