...
`awareness-space` が行っているのは、
既存の理論資産を単純に保管し直すことではありません。
意識をどのような単位で記述できるかを、新しい探索として立ち上げることが目的です。
...
この方法では、
source と公開文書を分離し、
既存素材の語彙をそのまま見せるのではなく、
`awareness-space` 単独で読める記述へ組み替えることを重視しています。
...
公開向け文書はそこから独立した形で書き直していく必要があります。
